...
const galleryCards = images.map(gallery => createGalleryCards(gallery)).join('');

const galleryListElement = document.querySelector('.gallery');

galleryListElement.innerHTML = galleryCards;
...
galleryListElement.addEventListener('click', event => {
  ...
  const galleryInstance = basicLightbox.create(`
    <img class="gallery-modal-img" src="${originalImageSrc}" alt="${target.alt}" />
  `);
